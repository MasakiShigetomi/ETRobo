...
Scene* Scenario::next() {
    if (mCurrentScene != 0) {
        mCurrentScene = mCurrentScene->next; //シーンをひとつ進める
    } else {
        mCurrentScene = mTopScene; //最初に実行、シーン1を現在のシーンに設定
    }
...
SceneCommands Scenario::currentSceneCommand() const {
    return (mCurrentScene == 0) ? GO_FORWARD : mCurrentScene->command;
}
 ...
 * @return タスク
 */
SceneTasks Scenario::currentSceneTask() const {
    return (mCurrentScene == 0) ? TIME_OUT : mCurrentScene->taskcom;
}
