...
void RandomWalker::execLineTracing() {
    ...
    if (mTransitionCount == 0) {
        mDistanceTracker->SetDistance(10000000);//ライントレース終了距離　※ほぼ永遠にライントレースする
    }
    mLineTracer->run();
     
    if (mDistanceTracker->ReachedDistance()) {//終了距離到達時
        mState = SCENARIO_TRACING;
        mTransitionCount++;  // 状態遷移の回数をインクリメント
        mSimpleTimer->setTime(1650000);//1.65秒間タイマーセット やたら桁が多いのはマイクロ秒のため
        mSimpleTimer->start();
    }
