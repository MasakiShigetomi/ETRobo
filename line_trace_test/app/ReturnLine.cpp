...
/* 関数宣言 */
// bool_t ReturnLine::ReturnLine_serchLine(void); // ライン探索する関数
/* 構造体定義 */
...
#define SET_DIRECTION_dirDiff 45   // 許容される最大旋回範囲
/* 関数宣言 */
// bool_t ReturnLine::ReturnLine_setDirection(bool_t edge); // 方位を調整する関数
// void ReturnLine::ReturnLine_setDirection_init(void);     // 初期化関数
/* 変数宣言 */
// ReturnLine::setDirection_enum setDirection_state;
...
/* 変数宣言 */
// ReturnLine::discernLine_enum discernLine_state; // 状態遷移関数
/***********************************************/
ReturnLine::ReturnLine() : colorSensor(PORT_2)
{
}
...
ライン復帰初期化関数
*************************************/
void ReturnLine::ReturnLine_init(void)
{
    /* ライン復帰関数 */
    ...
    /* 計測器初期化 */
    direction1.Direction_init();
    distance1.Distance_init();
}
...
true:ライン復帰完了
*************************************/
bool_t ReturnLine::ReturnLine_do(bool_t edge)
{
    // printf("serchLine_count = %d\n", serchLine_count);
...
true：ライン探索完了
*******************************************/
bool_t ReturnLine::ReturnLine_serchLine(void)
{
    float cur_dir = direction1.Direction_getDirection();
    float cur_dis = distance1.Distance_getDistance();
    // printf("cur_dir = %f\n", cur_dir);
    // printf("cur_dis = %f\n", cur_dis);
    ...
    if (ReturnLine_discernLine() == true)
    {
        ...
        else if (serchLine_state == SERCH_R)
        {
...
true：方位の調整終了
***************************************/
bool_t ReturnLine::ReturnLine_setDirection(bool_t edge)
{
    float cur_dir = direction1.Direction_getDirection();
    // float check_dir = 0.0;
...
車体の方位を調整する　初期化関数
******************************************/
void ReturnLine::ReturnLine_setDirection_init(void)
{
    setDirection_predir = direction1.Direction_getDirection();
...
true：ライン検知済み
******************************************/
bool_t ReturnLine::ReturnLine_discernLine(void)
{
    int color_number = colorSensor.getColorNumber();
...
ラインを検知する 初期化関数
****************************************/
void ReturnLine::ReturnLine_discernLine_init(void)
{
    discernLine_state = DISCERN_WHITE1;
}

///  testmethod
void ReturnLine::testmethod(void)
{
    printf("testmethod\n");
}